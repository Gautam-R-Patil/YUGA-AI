...
export const ClassroomViewRight: React.FC = () => {
    ...
    const lectureManager = {
        ...
        currentSpeech: isLessonActive ? "Notice how the field strength decreases with distance in this inverse-square relationship." : "Hello! Ready to start?",
        currentSubtitle: isLessonActive ? "Notice how the field strength decreases with distance." : "",
    ...
    return (
        <div className="absolute inset-0 bg-gradient-to-br from-[#0a0520] via-[#1a0f2e] to-[#0f1a2e] p-6 flex gap-6 overflow-hidden font-sans text-gray-100">
            ...
            {/* RIGHT COLUMN: Smart Board (Wide - 70%) */}
            <div className="flex-[7] flex flex-col min-w-0 bg-slate-900/50 backdrop-blur-xl rounded-3xl border border-white/5 overflow-hidden shadow-2xl relative">
                <div className="absolute inset-0 bg-gradient-to-r from-blue-900/5 to-transparent pointer-events-none"></div>
                <EnhancedSmartBoard
                    content={['# Inverse Square Law', '- Formula: F = k * (q1*q2)/r^2', '- Field Intensity vs Distance', '- Graphical Representation']}
                    lessonTitle={MOCK_LESSON.title}
                    currentSegment={""}
                    isActive={isLessonActive}
                    explanation={"Graph showing exponential decay of field strength as distance increases."}
                    isExplaining={false}
                    isAudioPlaying={isLessonActive}
                    highlightedText={''}
                    hasSections={true}
            ...
            {/* RIGHT COLUMN: Sidebar (30%) - Stacked Avatar & Chat */}
